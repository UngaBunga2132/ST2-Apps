...
''' Iteration Conversion 1'''
def sum_of_natural_numbers(n):
    return n * (n + 1) // 2

# Example usage:
t = 17
result = sum_of_natural_numbers(t)  # Calculates 1 + 2 + 3 + 4 + 5
print(f"The sum of the first {t} natural numbers is {result}")
...
# Example usage:
g = 30
result = fibonacci(g)  # Calculates the 7th Fibonacci number (1, 1, 2, 3, 5, 8, 13, ...)
...
# Compare times
insertion_time = timeit.timeit(lambda: insertion_sort(large_random_list[:]), number=15)
merge_time = timeit.timeit(lambda: merge_sort(large_random_list[:]), number=15)
...
l = [2, 6, 9, 5, 3, 4, 5, 6, 6, 7, 9, 8, 3, 5, 8] # [random.randint(1, 10) for _ in range(10)]
print(l)
print()
sorted = merge_sort(l)
print(sorted)
